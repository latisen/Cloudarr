...
Or if using LoadBalancer with `loadBalancerIP: 192.168.30.59`:

```
http://192.168.30.59:8080
```

Check API health:

```bash
curl http://192.168.30.59:8080/api/health
```
...
The deployment includes a LoadBalancer service on `192.168.30.59`. If this IP is unavailable or you want to change it:
